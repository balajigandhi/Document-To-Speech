...
(function () {
  ...
  let panel = null;
  let playBtn, progressRow, progressFill, progressLabel, bodyEl;
  let spans = [];
  ...
  function buildPanel(sentences) {
    ...
    header.append(title, playBtn, stopBtn, closeBtn);
    ...
    panel.append(header, controls, progressRow, bodyEl);
  ...
  function makeDraggable(el, handle) {
    ...
    handle.addEventListener('mousedown', e => {
      ...
      el.style.right = 'auto';
      const onMove = e => { el.style.left = (e.clientX - ox) + 'px'; el.style.top = (e.clientY - oy) + 'px'; };
      const onUp   = () => { document.removeEventListener('mousemove', onMove); document.removeEventListener('mouseup', onUp); };
      document.addEventListener('mousemove', onMove);
      document.addEventListener('mouseup', onUp);
    });
